...
class MCTS:
    def __init__(self, agent, initial_state, ori_text, tool_list, c_param=0, value_threshold=0, reflexion_threshold=10):
        # Initialize with exploration parameter (c_param)
        self.agent = agent
        self.c_param = c_param  # c_param 1 (selection 里面的parameter)
        self.value_threshold = value_threshold  # TODO jjh-兜底策略选出top value的值
        self.root = Node(state=initial_state, ori_text=ori_text, depth=0)  # Use initial_state for root node

        init_ppl, init_bart = self.agent.get_init_ppl_bart(ori_text)
        self.root_ppl = init_ppl
        self.root_bart = init_bart
        self.root.set_tool_lists(tool_list)
        self.root.set_history_metrics(ppl_list=[init_ppl], bart_list=[init_bart], some_list=[0], cons_list=[0])
        self.root_tool_list = tool_list
        # self.root.set_init_ppl_bart(init_ppl, init_bart)
        # print(self.root.)
    ...
    def expansion(self, simulation_iter, leaf_node, leaf_node_layer):
        #### todo: change here
        ...
        querying_results = self.agent.perform_querying(simulation_iter, leaf_node, leaf_node_layer, self.root_ppl, self.root_bart, self.root_tool_list)

        #### todo: change here
        for new_node, reward in querying_results:
    ...
    def run_search(
        ...
        for num in range(1, num_simulations+1):
            ...
            self.expansion(num, leaf_node, leaf_node_layer)
