...
class OpenAIModel:
    ...
    def vote_query(self, messages):
        retry = 0
        while retry < self.max_retry:
            try:
                response = self.generate(messages)
                print("-" * 3 + "vote_query_prompt" + "-" * 3)
                print(messages[-1]["content"])
                print("-" * 3 + "vote_query_response" + "-" * 3)
                print(response)
                print("-" * 3 + "end" + "-" * 3)
                score = extract_answer(response)
                if score == "None":
                    retry += 1
                    print(f"【No extracted score】\n{response}")
                else:
                    score = float(score)
                    return score
    ...
    def vote_answer(self, messages):
        retry = 0
        while retry < self.max_retry:
            try:
                response = self.generate(messages)

                print("-" * 3 + "vote_answer_prompt" + "-" * 3)
                print(messages[-1]["content"])
                print("-" * 3 + "vote_answer_response" + "-" * 3)
                print(response)
                print("-" * 3 + "end" + "-" * 3)

                score = extract_answer(response.split("[Answer]")[0])
                answer = response.split("[Answer]:")[-1].replace("\n", " ").strip()
                if score == "None":
                    retry += 1
                    print(f"【No extracted score】\n{response}")
                else:
